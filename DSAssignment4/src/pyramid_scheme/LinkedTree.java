...
public class LinkedTree<T> implements TreeADT<T> {
    ...
    public void addChild(T parent, T child) throws ElementNotFoundException {
        // locate parent node
        MultiTreeNode<T> node = findNode(parent);
        // if no nodes, return empty
        if(child == null || parent == null){
            return;
        }
        // throws exception if no parent node was found
        if (node == null) {
            throw new ElementNotFoundException("No such element");
        }
        // add child element to parent node
        addChild(node, child);
    }
    ...
    protected void addChild(MultiTreeNode<T> parentNode, T child) {
        MultiTreeNode<T> newChild = new MultiTreeNode<T>(child);
    ...
    @Override
    public int size() {
        // if root is null then size is 0
        if (root == null) {
            return 0;
        }
    ...
    private int countDown(MultiTreeNode<T> node) {
        // counts the root, even if no children exist
